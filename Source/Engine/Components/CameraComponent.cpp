...
namespace neu {
	...
	void CameraComponent::Read(const serial_data_t& value) {
		...
		if (!SERIAL_READ(value, aspect)) aspect = (float)GetEngine().GetRenderer().GetWidth() / (float)GetEngine().GetRenderer().GetHeight();
	...
	void CameraComponent::UpdateGui() {
		...
		if (projectionType == ProjectionType::Perspective) {
			ImGui::DragFloat("FOV", &fov, 1.0f, 10.0f, 100.0f);
		}
		else
		{
